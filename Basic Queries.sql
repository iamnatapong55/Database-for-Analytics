...
--Q1.1
SELECT emp.FNAME, emp.MINIT, emp.LNAME
FROM EMPLOYEE sup, EMPLOYEE emp
WHERE sup.SSN = emp.SUPER_SSN
AND sup.FNAME = 'Franklin'
AND sup.MINIT = 'T'
AND sup.LNAME = 'Wong';

--Q1.2
SELECT pro.PNAME, pro.PNUMBER, SUM(work.HOURS) AS TOTAL_HRS
FROM PROJECT pro, WORKS_ON work
WHERE pro.PNUMBER = work.PNO
GROUP BY pro.PNAME, pro.PNUMBER;

--Q1.3
SELECT dep.DNUMBER, dep.DNAME, AVG(emp.SALARY) AS AVERAGE
FROM DEPARTMENT dep, EMPLOYEE emp
WHERE dep.DNUMBER = emp.DNO
GROUP BY dep.DNAME, dep.DNUMBER
ORDER BY dep.DNUMBER ASC;
...
--Q1.5
SELECT dep.DNAME, COUNT(emp.DNO) AS NumOfEmployee
FROM DEPARTMENT dep, EMPLOYEE emp
WHERE dep.DNUMBER = emp.DNO
GROUP BY dep.DNAME
...
SELECT FNAME, MINIT, LNAME, SEX
FROM EMPLOYEE
WHERE SEX IN (SELECT SEX FROM EMPLOYEE WHERE SEX = 'M');
...
FROM EMPLOYEE
MINUS
--List employee who are assigned in any project
SELECT FNAME, MINIT, LNAME 
FROM EMPLOYEE
